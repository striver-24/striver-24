<div align="center">
  <img src="https://github.com/striver-24/striver-24/blob/main/Aesthetic%20Twitter%20Header.png" width="100%"/>

  # Hi, I'm Deivyansh Singh 👋
  **BTech Data Science & Engineering @ MIT Jaipur**
  
  [![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/deivyansh-singh)
  [![Portfolio](https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white)](https://deivyansh.vercel.app/)
  [![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:work.deivyansh@gmail.com)
  
  ### 🤖 Machine Learning | Agentic AI | SaaS Developer
  **Currently seeking opportunities to build the future of autonomous systems.**
</div>

---

## 🧠 About Me
I am a Data Science enthusiast specializing in **Agentic AI** and **LLM Orchestration**. My focus is on bridging the gap between raw machine learning models and scalable, production-ready SaaS solutions.

- 🏢 **Previous Internships:** IOCL, Panacea Infosec, & CareerSimplified.
- 🛠️ **Current Focus:** Building autonomous software agents and Mental Health Tech.
- 🌱 **Passionate about:** Open Source contribution and streamlining workflows with AI.

---

## 🛠️ Tech Stack

| Category | Tools & Technologies |
| :--- | :--- |
| **Languages** | Python, JavaScript (Node.js), Bash |
| **AI / ML** | PyTorch, TensorFlow, Keras, Scikit-learn, LangGraph, LangChain |
| **Data** | Pandas, NumPy, MySQL, MongoDB |
| **Web** | React, FastAPI, Streamlit, TailwindCSS, SASS |
| **DevOps** | AWS, Git, Docker, CI/CD (GitHub Actions) |

---

## 🚀 Featured Projects

### 🧠 [Momentum](https://github.com/striver-24/Momentum) 
*Autonomous "Flow-State" Engineering Agent.*
- Automates the development loop from natural language to production-ready Pull Requests.
- **Tech:** Agentic AI, LLMs, Python.

### ⚕️ [MedAppointment.io](https://github.com/striver-24/medical-appointment-agent)
*AI-driven Healthcare Orchestrator.*
- Handles patient intake, insurance verification, and scheduling using **LangGraph**.
- **Tech:** LangChain, Twilio API, Python.

### 😊 [MindSpace](https://github.com/striver-24/GenAI-Hackathon)
*GenAI Wellness Platform.*
- Culturally grounded AI companion for mental health check-ins.
- **Tech:** Next.js 14, Google Gemini, Firestore.

---

## 📊 Upcoming Projects

### ✎ [Sketch2Design](https://github.com/striver-24/sketch-to-design)
*App to convert your raw ideas into professional app design.*
**Coming Soon....**

---

<div align="center">
  <h3>Let's build something intelligent.</h3>
  <a href="https://x.com/singhdeivyansh">Twitter (X)</a> • <a href="https://medium.com/@deivyanshs">Medium</a> • <a href="https://instagram.com/singhdeivyansh">Instagram</a>
</div>

- 🌐 [Portfolio](https://deivyansh.vercel.app/)
- 💼 [LinkedIn](https://www.linkedin.com/in/deivyansh-singh)
- 📸 [Instagram](https://www.instagram.com/singhdeivyansh)

---

_“Striving for excellence in every line of code.”_

© Deivyansh Singh, 2026
